---
title: "HSF Coordination Meeting #298, 20 November 2025"
layout: plain_toc
---

## Attending

Present/Contributing: Claire Antel, Joe Osborn, Maarten van Veghel, Liz Sexton-Kennedy, Saptaparna Bhattacharya, Alexander Moreno, Michel Hernandez Villanueva, Ruslan Mashinistov, Torre Wenaus, Stephen Mrenna

Apologies/Contributing: Eduardo Rodrigues, Graeme Stewart

## News, general matters, announcements

### LHCC Meeting News
- A few recommendations:
    - Associate with more software projects - mostly passive when projects come to us. HSF should be more proactive in approaching projects directly.

### 2026 conveners

It’s that time of the year again to run the process to identify the 2026 activity area conveners.
- The call for nominations for 2026 conveners is open. Please nominate those you know

### HSF Seminar Series and Compute Accelerator Forum

Planned HSF seminars:
- EPPSU & Venice Symposium Seminar on [26th Nov](https://indico.cern.ch/event/1481829/): Announcement sent. 
- TBC: Seminar on HS3, 3rd Dec - haven't heard back from speakers.
- Open Data seminar (Trig & Reco group): [28th Jan 2026](https://indico.cern.ch/event/1604475/).

Others in the works (no updates):
- 2nd in series on AI-assisted SW tools organised by software tools & packaging
- Seminar from creator of mp-units (supports units and constants in C++, see AOB) - still to be arranged.

(Thanks to the activity groups for coming to us with seminar ideas and speakers!)

HSF seminar conveners are reachable at <mailto:hsf-seminar-conveners@googlegroups.com>.

Compute & Accelerator forums:
- 12 Nov: [A Formal but Pragmatic Foundation for General-Purpose Operating Systems](https://indico.cern.ch/event/1472688/).
    - seminar was excellent, recommend watching the recording

Please send your suggestions for next seminars.


### HSF Affiliated Projects and Software
- Pepper project review ongoing


## Activities Updates


### Software Training

Future Events:
- Blueprint Meeting on AI - TBD
    - Survey to collect comments in the scope of training. We will send the survey specifically to conveners of AI (for instance https://iml.web.cern.ch/forum)
    - Organize a dedicated session with different collaborations to define a roadmap on training
- We are organizing training events for next year. If you have any suggestions, comments or ideas, please let us know asap

Other activities:
- Finalizing the paper “Training on Data Analysis Reproducibility via Containerization with Apptainer” for The Journal of Open Source Education (JOSE). We will circulate a message with the draft to collect authorship information soon
- Maintenance of repositories

### Event generators
- Review for SHERPA pepper ongoing, similar to review for NNPDF
- Plan on submitting 2 GSoCs for negatively weighted events


### Reconstruction and Software Triggers
- Suggestion from IML to make the open data set seminar in January a joint HSF/IML seminar to broaden attendance. No objections from our side


### JuliaHEP

JuliaHEP 2026 will be in Munich. We are planning for the end of September or the beginning of October

JuliaCon 2026 - We are working on the JuliaHEP Mini-symposium proposal

We are organizing JuliaHEP Users Meetings - every other month next year


## AOB

### Physical Constants / HEPdata Library

Standing reminder: we proposed a C++ header-only library that defines physical constants and some salient HEPdata in a lightweight, easy-to-use manner. Also taking into account versioning.

Please get in touch with Graeme Stewart if you would like to discuss involvement in this.

### Next Meeting

The next coordination meeting will be December 4th, <https://indico.cern.ch/event/1477091/>, Ruslan will chair. Last of the year (?)

### Chair This Meeting 👇

Please [sign up](https://docs.google.com/spreadsheets/d/1Z1Z4payCpieOLiVFcC6y9j-KCj71u6xX232LHUgIHfI/edit) for chairing a future coordination meeting. (There is even a [HOWTO guide](https://hepsoftwarefoundation.org/organization/running-meetings.html)).
