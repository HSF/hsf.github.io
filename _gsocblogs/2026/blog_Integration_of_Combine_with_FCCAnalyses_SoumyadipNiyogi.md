...
My research so far has lived mostly in cosmology: [MCMC-based constraints on modified gravity](https://arxiv.org/abs/2408.03683), Bayesian inference on Gamma-Ray Burst datasets, and simulating relativistic electrons in galaxy cluster magnetic fields. This project is my first serious step into collider physics, and I could not have asked for a better starting point.

## The Project
The Future Circular Collider (FCC) programme at CERN uses FCCAnalyses as its primary analysis framework. Built on ROOT's RDataFrame, it efficiently processes simulated [EDM4hep](https://github.com/key4hep/EDM4hep) based events into histograms. But the next step, turning those histograms into actual physics results like discovery significances or exclusion limits, requires a statistical fitting tool, like CMS Combine. Combine is a separate statistical tool that FCC physicists currently have to interface with manually.

That manual step is what I am here to automate. By the end of the summer, the goal is to have a native Python interface inside FCCAnalyses that reads RDataFrame output histograms and automatically generates the datacards and RooFit workspaces that Combine needs. The first piece of work is packaging Combine itself inside the [Key4hep](https://github.com/key4hep) software stack so it is available as a standard dependency, requiring no separate installation.
...
On the FCCAnalyses side, I have been working through the benchmark ZμμHbb analysis tutorial. This involved executing the histmaker style analysis, stacking signal and background processes with the plotting utilities, and migrating my workspace from AFS to EOS to handle larger ROOT files. Generating recoil mass plots and studying b-tagging score distributions has given me a concrete picture of exactly what data structures my integration code will need to consume.
...
I will also be attending the group meeting with the CERN EP-FCC group team to introduce myself to everyone else, which should be highly beneficial for my future workflow and collaboration.
